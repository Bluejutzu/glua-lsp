...
- 803c677: Local module/OOP tables (`local Handler = {}` with `function Handler:Method()` or `Handler.field = ...`) now get completion, hover and argument-type checking for methods and fields added after the initial `{}` — previously only genuine globals tracked this. `self` inside such a method also sees sibling methods defined later in the same file.
