...
Every file in your workspace gets a realm — Client, Server, or Shared — based on its path, narrowed further inside `if SERVER` / `if CLIENT` blocks. Call an API that doesn't exist in the current realm and you get a warning for it.
