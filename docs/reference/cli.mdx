...
This repository publishes [pre-commit](https://pre-commit.com) hooks, so
`glua lint` and `glua fmt` can run on staged files before a commit lands:
...
Neither hook needs Node or `glua-cli` installed beforehand — pre-commit
installs `glua-cli` itself the first time the hook runs, in its own isolated
environment. `rev` pins which version of the *hook definition* you get, not
which version of `glua-cli` gets installed; add this under a hook to pin that
too:

```yaml
      - id: glua-lint
        additional_dependencies: ['glua-cli@0.5.4']
```

Both hooks still index the whole project even when only a few files are
staged — cross-file rules need the whole picture — which is exactly the case
the [fact cache](#caching) exists to make cheap.
