...
See the [glua.schema.json reference](/reference/glua-schema) for every property, including `workspace`, `completion`, `inlayHints` and `hover`.
...
See the [`gluafmtrc.schema.json` reference](/reference/gluafmtrc-schema) for every property, including `operatorStyle`, `commentStyle` and `endOfLine`.
