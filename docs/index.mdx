...
GLua for Garry's Mod is a VS Code extension for GMod Lua. It ships with 5,586 API entries from the Garry's Mod wiki, so completions, type tracking, and diagnostics work without an internet connection. It also covers GMod-specific concepts other Lua tooling misses: server/client/shared realms, net message payloads, and hook signatures.
...
GLua does not claim every `.lua` file on your machine. It activates only on Garry's Mod workspaces, or when you explicitly enable it. The status bar shows the current mode:
