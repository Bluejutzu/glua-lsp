...
GLua Language Server is distributed through GitHub Releases, so you install the `.vsix` file directly. You can also build it yourself from source. Both methods take only a few minutes.

<Note>
  Every release is published to GitHub with the packaged `.vsix` attached. Registry publishing to [Open VSX](https://open-vsx.org) — the registry Cursor, VSCodium and Gitpod install from — is set up but not yet enabled; until then, use the `.vsix` below.
</Note>
