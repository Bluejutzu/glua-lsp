...
docs [changelog page](/glua/changelog) from it, so the two never drift apart.
...
pnpm run build
pnpm run package
npx ovsx publish packages/glua-lsp/glua-gmod-0.1.0.vsix --pat <token>
```
...
pnpm run build
pnpm run package
pnpm --filter glua-gmod exec vsce publish --no-dependencies --packagePath glua-gmod-0.1.0.vsix
```
...
```bash
pnpm --filter glua-gmod package:ls
```
