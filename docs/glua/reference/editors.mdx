...
formatting](/glua/reference/cli), and `glua-lsp` for the language server. The wiki
...
[configuration page](/glua/configuration/overview) applies.

If your client does not send configuration, the server falls back to its
defaults, and a committed [`.glua.json`](/glua/configuration/config-files) still
