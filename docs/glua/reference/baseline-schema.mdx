...
[Adopting on a codebase you inherited](/glua/reference/cli#adopting-on-a-codebase-you-inherited)
...
<ParamField path="$schema" type="string">
  URL of the JSON schema for this file. Written automatically by `glua lint --suppress-all`.
</ParamField>

<ParamField path="generated" type="string">
  When the baseline was last written, as an ISO timestamp. For a human reading a diff — never read back.
</ParamField>

<ParamField path="files" type="object" required>
  Project-relative file path → rule code → number of findings accepted for it. See [Shape of `files`](#shape-of-files).
</ParamField>

## Shape of `files`

Keyed by file path, then by the **diagnostic code** (the kebab-case name shown
in the Problems panel and listed on the [rules reference](/glua/reference/rules) —
...
<Warning>
  This file is generated. Hand-editing a count down is indistinguishable from the code actually improving, which is exactly what `--prune-suppressions` exists to do safely — see the [pruning note](/glua/reference/cli#adopting-on-a-codebase-you-inherited).
</Warning>
